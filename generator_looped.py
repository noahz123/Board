...
4. Run continuously to generate new boards daily at midnight Eastern Time
"""
...
def generate_daily_board():
    """
    Generate a new board, solve it, and upload the results.
    This function is meant to be called by the scheduler.
    """
    print(f"\n[{datetime.datetime.now()}] Running daily board generation...")
    ...
    print(f"[{datetime.datetime.now()}] Daily board generation completed")
    
    # Return the date the board was generated
    return datetime.datetime.now().strftime('%Y-%m-%d')

def run_scheduler():
    """Set up and run the scheduler for continuous operation."""
    # Configure the scheduler to run at midnight Eastern Time
    eastern = pytz.timezone('US/Eastern')
    
    def get_midnight_time():
        """Get the time string for midnight Eastern Time"""
        now = datetime.datetime.now(eastern)
        midnight = now.replace(hour=0, minute=0, second=0, microsecond=0)
        if now >= midnight:
            # If it's already past midnight, schedule for the next day
            midnight = midnight + datetime.timedelta(days=1)
        return midnight.strftime('%H:%M')
    
    # Schedule the job to run at midnight Eastern Time
    schedule.every().day.at(get_midnight_time()).do(generate_daily_board)
    
    # Initialize with a first run
    today = datetime.datetime.now(eastern).strftime('%Y-%m-%d')
    print(f"Initial board generation for {today}")
    board_date = generate_daily_board()
    ...
    print(f"\nScheduler is running. Next board will be generated at midnight Eastern Time.")
...
def main():
    ...
    parser.add_argument('action', choices=['generate', 'solve', 'both', 'upload', 'schedule'], 
                        help='Action to perform: generate a board, solve a board, both, upload files, or run scheduler',
    ...
    if args.action == 'schedule':
        run_scheduler()
    elif args.action == 'generate' or args.action == 'both':
        generate_and_save()
        print()
